...
export default function TweetComponent(tweetContent: any) {
  ...
  const handleLike = async(id:string)=>{
    const response = await axios.post(`http://twitter-backend-rcbd.onrender.com/tweet/like/${id}`)
  ...
  function createTweet(tweetString: string): any {
    ...
    tweet.forEach((word: string) => {
      ...
      if (word.startsWith("#")) {
        ...
        link.href = `http://twitter-backend-rcbd.onrender.com/tweet/${word2}`
    ...
    >
      ...
      <div id="user-info" className="basis-5/6 w-96 mr-5">
        ...
        >
          ...
          <a href={`http://twitter-backend-rcbd.onrender.com/comments/${tweetid}`} className="flex w-24 text-slate-500 hover:text-blue-600">
